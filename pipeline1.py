...
my_list = [
    "HumanEval/1",  # Add more problem IDs here
]
...
def generate_attempts(problem_prompt, num_samples=3):
    ...
    for i in range(num_samples):
        ...
        final_prompt = prefix_prompt + problem_prompt + suffix_prompt
        text = groq_generate(final_prompt, temperature=1.0)
...
for idx, (key, problem) in enumerate(selected_problems.items(), start=1):
    ...
    final_prompt = prefix_prompt + problem['prompt'] + suffix_prompt
    ...
    # Evaluate correctness for the two main attempts
    ...
    # Generate additional random samples for evaluation
    ...
    # Incrementally append to CSV after each problem
